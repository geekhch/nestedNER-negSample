...
from utils import corpus_to_iterator, Procedure, GPU_ID, strftime
...
if __name__ == "__main__":
    ...
    parser.add_argument("--negative_rate", "-nr", type=float, default=0.5)
    ...
    lexical_vocab = UnitAlphabet(bert_name, args.cache_dir)
    ...
    checkpoint_path = os.path.join(args.check_dir, f"model.pt")
    # checkpoint_path = os.path.join(args.check_dir, f"model-neg0.3.pt")

    model = PhraseClassifier(lexical_vocab, label_vocab, args.hidden_dim,
                             args.dropout_rate, args.negative_rate, bert_name, args.cache_dir)
    ...
    for epoch_i in range(0, args.epoch_num + 1):
        ...
        # test_f1, test_time = Procedure.test(model, test_loader)
        # print("{{Epoch {:3d}}} f1 score on test set is {:.5f} using {:.3f} secs".format(epoch_i, test_f1, test_time))

        if dev_f1 > best_dev:
            best_dev = dev_f1
            print("\n<Epoch {:3d}> save best model to{} with score: {:.5f} in terms of dev set".format(epoch_i, checkpoint_path, dev_f1))
